...
function list_users($db_server) 
{
		...
		$content="";
			...
			$check_in_mysql="SELECT user.id,user.name,user.surname,user.father_name,sap.sap_id,user.email,
								user.tab_num,user.position,org_dpts.name,org_plants.name
							FROM user
							LEFT JOIN sap ON sap.user_id=user.id
							LEFT JOIN org_user_reg ON org_user_reg.user_id=user.id
							LEFT JOIN org_dpts ON org_user_reg.dpt_id=org_dpts.id
							LEFT JOIN org_plants ON org_dpts.plant_id=org_plants.id
							WHERE 1
							ORDER by org_plants.code,org_dpts.id,user.id ";
		...
		$content.= '<thead>';
		$content.= '<tr><th>№</th><th>ФИО</th><th>Подразделение</th><th>UserID</th><th>email</th>
					</tr>';
		$content.= '<tbody>';
		...
		{ 
				...
				$dpt=$row[8];
				$plant=$row[9];
				$content.= '<tr><td>'.$counter.'</td><td>'.$surname.' '.$name.' '.$fname.' <br><small>'.$pos.'</small></td><td>'.$plant.' | '.$dpt.' </td><td>'.$sap_id.'</td><td><a href="show_user.php?command=show&id='.$rec_id.'">'.$email.'</a></td></tr>';
...
/*
	...
	OUTPUT:  HTML TABLE
*/
function show_user($db_server,$id)
{
