...
/* 
	...
	INPUT: au/logs/pfm_by_account.csv
	OUTPUT:  pfm_by_acc TABLE
...
//Set up mySQL connection
	...
	$counter=0;	
	$fp = fopen('./au/logs/pfm_by_account.csv', 'r');
	$fout = fopen('./au/logs/pfm_missing_ccs.csv', 'w');
	if($fp)
	{
			$in='';
			$in= fgets($fp);
			...
			while(!feof($fp))
			{
				...
				if($flag)
				{
					$transfer_mysql='INSERT INTO pfm_by_acc
				...
				//echo $transfer_mysql.'<br/>';				
					...
					if(!$answsql3) die("INSERT into  costs_by_acc TABLE failed: ".mysqli_error($db_server));
					else $counter+=1;
				}
